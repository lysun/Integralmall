...
<my:admin tab="integral">
	...
	<jsp:attribute name="script">
		...
		function initialize() {
			...
			if('${t.longitude}'!="" && '${t.longitude}' !=null
					&&'${t.latitude}'!="" &&'${t.latitude}'!=null){
		...
		$(function(){
			...
			actualOper(document.getElementById('actual'));
			//如果是实物商品则隐藏部分输入框
		...
		function actualOper(obj){
			if(obj.checked){
				...
				$("#price").parent().parent().attr("style","display:block");
			}else{
				$("#expiryTime").parent().parent().attr("style","display:block");
				$("#address").parent().parent().attr("style","display:block");
				$("#shopName").parent().parent().attr("style","display:block");
				$("#point").parent().parent().attr("style","display:block");
				loadScript();
				$("#price").val("");
	...
	<jsp:body>
		<form action="./" role="form" class="form-horizontal" method="post" enctype="multipart/form-data" onsubmit="return validate()">
           ...
           <div class="form-group">
               ...
               <div class="col-sm-10">
               	<c:if test="${t.isActual eq '0'}"><input id="actual" name="isActual" type="checkbox" value="1"/></c:if>
               	<c:if test="${t.isActual eq '1'}"><input id="actual" name="isActual" type="checkbox" value="1" checked/></c:if>
               </div>
           </div>
           <div class="form-group">
           ...
           <div class="form-group">
               ...
               <div class="col-sm-10">
                   <input id="expiryTime" class="form-control" name="expiryTime" value="${t.expiryTime }" placeholder="例如:格式为2001-01-01">
               </div>
           </div>
           <div class="form-group">
           ...
           <div class="form-group">
               ...
               <div class="col-sm-10">
                   <input id="shopName" class="form-control" name="shopName" value="${t.shopName }">
               </div>
           </div>
           <div class="form-group">
               <label for="address" class="col-sm-2 control-label">地址:</label>
               <div class="col-sm-10">
                   <input id="address" class="form-control" name="address" placeholder="请输入商品的地址" value="${t.address }">
               </div>
           </div>
           <div class="form-group">
